...
## General Description
***
C195 (Software II) is a scheduling software that pulls from a mySQL database. In the database there are multiple tables: Appointments, Contacts, Countries, Customers, First Level Divisions, and Users.
...
Once you click run you may need to modify the run configuration. So, first make sure the SQL connector and JAVAFX are connected to the modules. You can do this by clicking FILE, Project Structure, Libraries, then add the Java Libraries, they can both be found on the desktop, add the .jar file from the "my-sql-connector-java-8.0.25" folder, and the lib folder from the "openjx-11.0.2_windows-x64_bin-sdk" folder. (You may get an error in the console when running, this is just a warning saying the project has a newer version of JavaFX - running Java17 - this is not an issue however, and the program will still run)
...
## Additional Report
***
The additional report I created shows two tables, the first table shows duration of all appointments, with the following columns the appointment ID, Start time, End time and Duration of the appointment. The second table has just one column, the average duration of all appointments. This reports can help show the length of appointments to decide if appointments should be shorter or longer to help the customer.
